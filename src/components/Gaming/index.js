...
class Gaming extends Component {
  ...
  getDetails = async () => {
    ...
    const response = await fetch('https://apis.ccbp.in/videos/gaming', opt)
  ...
  render() {
    ...
    return (
      <>
        ...
        <Main data-testid="gaming" color={ground ? '#0f0f0f' : '#f9f9f9'}>
          <Back1>
            <Back2>
              ...
              <li className="rabbit1" key="gaming">
                <Link to="/gaming" className="rabbit">
                  <SiYoutubegaming className="coloring" />
          ...
          <Main2 colour={ground ? '#181818' : '#e2e8f0'}>
            <Main3 colour={ground ? '#606060' : '#cbd5e1'}>
              <SiYoutubegaming
                className={ground ? 'black-class' : 'white-class'}
              />
              <Param colour={ground ? '#ffffff' : '#000000'}>Gaming</Param>
            </Main3>

            <ul className="unorder1">
              {matrix.map(each => (
                <Games item={each} key={each.id} backChange={ground} />
              ))}
            </ul>
          </Main2>
        </Main>
